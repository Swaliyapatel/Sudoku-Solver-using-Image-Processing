...
def preProcess(img):
    ...
    imgBlur = cv2.GaussianBlur(imgGray, (5, 5), 1)  # ADD GAUSSIANpip  install  BLUR
...
def getPredection(boxes,model):
    ...
    for image in boxes:
        ...
        img = img / 255
        img = img.reshape(1, 28, 28, 1)        #converting the training_images array to 4 dimensional array

        ## GET PREDICTION
        predictions = model.predict(img)             #Run the pre-trained model it return value from entire rows
        classIndex = model.predict_classes(img)
        probabilityValue = np.argmax(predictions, axis=-1)             #-1 -> operation is performed across the rows of log_preds
